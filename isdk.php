...
#########           Updated by Justin Gourley on 03-08-12        #########
#########           Version 1.7.7                                #########
##########################################################################
...
//////////CONNECTOR/////////////
////////////////////////////////
###Connect without an entry in the config file###
public function connectWithVars($appName, $key, $type, $dbOn="on") {
  $this->debug = $dbOn;
  if(isset($appName) && isset($key)){

  
    if ($type=="i") {
      $this->client = new xmlrpc_client("https://" . $appName .
".infusionsoft.com/api/xmlrpc");
    } elseif ($type=="t") {
      $this->client = new xmlrpc_client("https://" . $appName .
".infusiontest.com/api/xmlrpc");}
      elseif ($type=="m") {
      $this->client = new xmlrpc_client("https://" . $appName .
".mortgageprocrm.com/api/xmlrpc");
  ...
  else throw new Exception("Not all fields set in function");
